...
``rubicon_ml`` is available to install via ``conda`` and ``pip``. When using ``conda``,
...
``rubicon_ml`` has a few optional extras if you're installing with ``pip`` (these extras are all
...
to see ``rubicon_ml`` integrated into a simple Prefect flow.
